...
class ProgressModules(models.Model):
    SORT_LIST = ["progress_module", "display_name", "created", "course_id",
...
class ProgressModulesHistory(models.Model):
    ...
    def __repr__(self):
        return ("{} : created {}"
            ).format(self.progress_module, self.created)
    ...
    def save_history(sender, instance, **kwargs):
        history_entry = ProgressModulesHistory(progress_module=instance,
            created=instance.created, display_name=instance.display_name,
            count=instance.count, max_score=instance.max_score,
            total_score=instance.total_score, submit_count=instance.submit_count,
            weight=instance.weight, start=instance.start, due=instance.due,
            correct_map=instance.correct_map, student_answers=instance.student_answers)
        history_entry.save()
