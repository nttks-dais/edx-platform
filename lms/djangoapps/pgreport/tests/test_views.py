...
from pgreport.views import (ProgressReport, UserDoesNotExists, InvalidCommand,
...
class ProgressReportTestCase(ModuleStoreTestCase):
    ...
    def test_get_correctmap(self):
        ...
        self.assertEquals(corrects, {self.problems[0].location.url() + "_2_1": 1,
    ...
    def test_get_student_answers(self):
        ...
        self.assertEquals(answers1, {self.problems[0].location.url() + "_2_1": {"Correct": 1},
            self.problems[0].location.url() + "_2_2": {"Incorrect": 1}})
        self.assertEquals(answers2, {
            self.problems[1].location.url() + "_2_1": ANY})
        self.assertEquals(answers2[self.problems[1].location.url() + "_2_1"],
    ...
    def test_collect_module_summary(self):
        ...
        with patch('pgreport.views.ProgressReport._get_module_data',
            return_value=module_data) as pgmock:
            self.pgreport.collect_module_summary(module_mock)
        ...
        self.assertEquals(self.pgreport.module_summary[module_mock.location], {
            ...
            'submit_count': 1,
            'start': module_mock.start,
            'student_answers': {module_mock.location.url() + '_2_1': {'Correct': 1},
                module_mock.location.url() + '_2_2': {'answer1': 1, 'answer2': 2}},
            'correct_count': 2,
            'max_score': 4.0,
    ...
    def test_collect_student_summary(self):
        module_mock = MagicMock()
        module_mock.location = self.problems[0].location
        self.pgreport.collect_student_summary(module_mock, self.students[0], 
            {"grade":True, "percent":"1.0"})

        def student_summary(loc):
            return [
                loc,
                self.students[0].last_login.strftime("%Y/%m/%d %H:%M:%S %Z"),
                True,
                "1.0",
                module_mock.start,
                module_mock.display_name,
                {},
                module_mock.weight,
                {},
                module_mock.category,
                module_mock.due,
                module_mock.get_progress(),
            ]

        students_summary_header = [
            'location', 'last_login', 'grade', 'percent', 'start', 'display_name',
            'student_answers', 'weight', 'correct_map', 'type', 'due', 'score/total'
        ]

        self.assertEquals(self.pgreport.students_summary, {
            'students:summary:header': students_summary_header,
            self.students[0].username: [student_summary(module_mock.location)]
        })

        self.pgreport.collect_student_summary(module_mock, self.students[1], 
            {"grade":True, "percent":"1.0"})
        self.assertEquals(self.pgreport.students_summary, {
            'students:summary:header': students_summary_header,
            self.students[0].username: [student_summary(module_mock.location)],
            self.students[1].username: [student_summary(module_mock.location)]
        })

        module_mock.location = self.problems[1].location
        self.pgreport.collect_student_summary(module_mock, self.students[0], 
            {"grade":True, "percent":"1.0"})
        #self.fail(self.pgreport.students_summary)
        self.assertEquals(self.pgreport.students_summary, {
            'students:summary:header': students_summary_header,
            self.students[0].username: [student_summary(self.problems[0].location),
                student_summary(self.problems[1].location)],
            self.students[1].username: [student_summary(self.problems[0].location)],
        })
    ...
    def test_get_children_rec(self):
        ...
        self.assertEquals(self.pgreport.location_parent, [
            {self.problems[0].location: [self.chapter.display_name,
                self.section.display_name, self.vertical.display_name]},
            {self.problems[1].location: [self.chapter.display_name,
                self.section.display_name, self.vertical.display_name]},
        ])
    ...
    @patch('pgreport.views.ProgressReport.collect_student_summary')
    def test_get_raw(self, csmock, cmmock, camock, symock):
        ...
        with nested(
            ...
            students = self.pgreport.get_raw(command="students")
        ...
        with nested(
            ...
            self.pgreport.location_list = location_list
            self.pgreport.module_summary = module_summary
            summary, students, modules = self.pgreport.get_raw()
    ...
    def test_create_pgreport_csv(self):
        students_summary = {
            "students:summary:header":["loc", "last_login"],
        ...
        progress_mock = MagicMock()
        progress_mock.get_raw.return_value = students_summary
        scontent_mock = MagicMock()
        cstore_mock = MagicMock()
        enter_mock = MagicMock()
        enter_mock.__enter__.return_value = self.output
        #enter_mock.__exit__.return_value = None
        enter_mock.__exit__.return_value = False
        ...
        smock.assert_called_once_with(ANY, "students-progress", "text/csv", "dummy-data")
        ...
        header = "username,loc,last_login\r\n"
        student1 = "{},{},{}\r\n".format(
            self.students[0].username, self.problems[0].location.url(), "2014/1/1")
        student2 = "{},{},{}\r\n".format(
            self.students[1].username, self.problems[1].location.url(), "2014/1/1")
        self.assertEquals(self.output.getvalue(), header + student2 + student1)
    ...
    def test_delete_pgreport_csv(self):
        ...
        scmock.assert_called_once_with(ANY, "students-progress", "text/csv", "dummy-data")
    ...
    def test_get_pgreport_table(self):
        ...
        self.assertEquals(summary, {'enrollments': 7, 'active_students': 6,
            'module_tree': [
    ...
    def test_update_pgreport_table(self):
        ...
        pmock.assert_any_call(count=6, display_name=self.problems[0].display_name,
...
class ProgressModulesTestCase(TestCase):
    ...
    def test_repr(self):
        self.assertEquals(str(self.pgmodule),
            "[ProgressModules] i4x://org/cn/problem/unitid")

    def test_unicode(self):
        self.assertEquals(unicode(self.pgmodule),
            "[ProgressModules] i4x://org/cn/problem/unitid")
...
class ProgressModulesHistoryTestCase(TestCase):
    ...
    def test_repr(self):
        self.assertEquals(str(self.phmodule),
            "[ProgressModules] i4x://org/cn/problem/unitid : created {}".format(self.created))

    def test_unicode(self):
        self.assertEquals(unicode(self.phmodule),
            "[ProgressModules] i4x://org/cn/problem/unitid : created {}".format(self.created))
